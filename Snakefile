...
rule splits:
    ...
    run:
        ...
        for train_idx, test_idx in gss.split(indices, y):
            ...
            ser_indices_test = df_indices.iloc[test_idx, :]["idx"]
            df_test = pd.concat([
                df_test,
...
rule kappa_error_all:
    ...
    run:
        ...
        res = []
        for ((e1, clf_1), X_test_1), ((e2, clf_2), X_test_2) in \
                itertools.combinations(zip(eclf.estimators_,X_test_list), 2):
...
rule ensemble_rnd:
    ...
    run:
        ...
        idcs = pd.read_csv(input[3], index_col=0)["enc_index"]
        train_paths = list(set(
